...
define('HeaderContentmd5', 'X-Sdk-Content-md5');
...
class Signer
{
    ...
    private function CanonicalRequest($request, $signedHeaders)
    {
        ...
        $CanonicalURI = str_replace(['%252F'], ['/'], $CanonicalURI);
        ...
        $hash = $this->findHeader($request, HeaderContentmd5);
        if (!$hash) {
            $hash = hash('md5', $request->body);
        }
    ...
    private function StringToSign($canonicalRequest, $t)
    {
        ...
        $hash = hash('md5', $canonicalRequest);
    ...
    {
        return hash_hmac('md5', $stringToSign, $signingKey);
    }
    ...
    private function AuthHeaderValue($signature, $accessKey, $signedHeaders)
    {
        ...
        return "SDK-HMAC-md5 Access=$accessKey, SignedHeaders=$signedHeadersString, Signature=$signature";
    }
